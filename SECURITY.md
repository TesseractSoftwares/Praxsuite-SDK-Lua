...
- the SDK version and engine version (Roblox Studio, FiveM, ...)
...
- A secret key reaching the client half of a game (in Roblox terms, anything a LocalScript can read)
- Credentials stored or transmitted less carefully than documented
...
- Keep the SDK **server-side only**. On Roblox that means ServerScriptService, never a
  LocalScript - `realm = "server"` in `wally.toml` is a hint, not an enforcement.
- Store the API key in the Roblox Secrets Store (`apiKeySecret`), not as a literal. The raw
  `apiKey` option exists for Studio testing and should not reach a published game.
- Scope the credential to the minimum tables the game actually needs.
- Per-player tables need an `Enduser` column with a `__SELF__` row filter **and** a
  `{{claim:sub}}` default value template - both, not one. With only the filter, inserts land
  with a null owner that the filter then hides, so a player saves and cannot read it back.
- Currency, inventory and score writes belong behind gateway endpoints, not direct table writes.
